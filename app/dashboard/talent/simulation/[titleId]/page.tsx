...
interface SimulationItem {
  ...
  start_problem_statement_id: string;
  id_title: string;
}

export default function SimulationsListByTitlePage({
  params,
}: {
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2].map((n) => (
              <div
                key={n}
                className="h-48 bg-gray-900 animate-pulse rounded-xl border border-gray-800"
              />
            ))}
          </div>
